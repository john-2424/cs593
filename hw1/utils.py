...
def evaluate_policy(policy, obs_space, env_name, episodes = 5, return_frames = False, max_length = None):
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            obs_vec = torch.from_numpy(encode_obs(obs, obs_space)).unsqueeze(0)

            with torch.no_grad():
                logits = policy(obs_vec)
                # TODO: Select the action with highest probability from logits
                # Note: you can also turn this into a probability distribution (Hint: softmax) and sample an action from it.
                # Greedy (deterministic) pick:
            ...
            # TODO: Step the environment using the selected action and accumulate reward. Refer to the Gymnasium API and the step() function.
